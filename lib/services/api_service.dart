...
class ApiService {
  ...
  // Add a method specifically for background pinging
  Future<void> backgroundPing() async {
    try {
      final licenseKey = await getStoredLicenseKey();
      if (licenseKey == null || licenseKey.isEmpty) {
        commonPrint.log('No license key available for background ping');
        return;
      }
      
      final deviceId = await _getDeviceId();
      
      // Create a new WebSocket connection just for this ping if needed
      WebSocketChannel? tempWebSocket;
      
      try {
        final wsUri = Uri.parse('$wsUrl/ws/$licenseKey/$deviceId');
        commonPrint.log('Background ping: Connecting to WebSocket: $wsUri');
        
        tempWebSocket = WebSocketChannel.connect(wsUri);
        
        // Wait for connection to establish
        await Future.delayed(const Duration(seconds: 1));
        
        // Send ping
        tempWebSocket.sink.add(jsonEncode({'type': 'pong', 'background': true}));
        
        // Wait for response (optional)
        await Future.delayed(const Duration(seconds: 1));
        
        // Close the connection gracefully
        tempWebSocket.sink.close();
        commonPrint.log('Background ping: WebSocket ping completed');
      } catch (e) {
        commonPrint.log('Background ping: WebSocket error: $e');
      ...
      commonPrint.log('Background ping error: $e');
    }
  }

  // When app is put in foreground, ensure WebSocket is connected
  Future<void> checkAndReconnectWebSocket() async {
    if (_licenseKey == null || _webSocketChannel == null) {
      final licenseKey = await getStoredLicenseKey();
      final loggedIn = await isLoggedIn();
      
      if (loggedIn && licenseKey != null && licenseKey.isNotEmpty) {
        _licenseKey = licenseKey;
        _connectWebSocket();
        commonPrint.log('Reconnected WebSocket after app resumed');
      }
    } else {
      ...
      try {
        ...
        commonPrint.log('WebSocket connection error on resume: $e');
        _handleWebSocketReconnect();
      }
    }
  }
